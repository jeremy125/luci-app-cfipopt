...
打开 LuCI 页面时自动检查 GitHub Release（服务端 1 小时缓存，避免频繁请求 API），
状态卡片显示「当前版本 / 最新版本」；有新版时「立即更新」按钮可用，
一键下载 ipk → opkg 安装 → 自动重启 rpcd/uhttpd，完成后刷新页面即可。
也可随时点「检查更新」手动触发。
...
- **2026-08-13 (v1.0.3)** — 更新检查支持强制刷新(手动「检查更新」/「立即更新」绕过 1h 缓存)
- **2026-08-13 (v1.0.2)** — 新增自动更新检查: 自动对比 GitHub Release 版本, 一键在线更新
- **2026-08-13 (v1.0.1)** — 修复 LuCI 界面配置表单未渲染的问题（`m.render()` Promise 未挂载），配置项（来源/数量/端口等）现可正常使用
- **2026-08-13 (v1.0.0)** — 首个可用版本: 优选/测速/绕过代理/结果导出全链路
